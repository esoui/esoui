...
function ZO_Gamepad_ParametricList_BagsSearch_Screen:IsQuestItemInSearchTextResults(questItemId)
    if self.searchContext then
        return TEXT_SEARCH_MANAGER:IsDataInSearchTextResults(self.searchContext, BACKGROUND_LIST_FILTER_TARGET_QUEST_ITEM_ID, questItemId)
    end
    -- Return true for every result if we don't have a context search
    return true
end
