            ...
            if DoesPlayerHaveGuildPermission(self.guildId, GUILD_PERMISSION_MANAGE_BLACKLIST) then
                local dialogData = 
                {
                    ...
                    changedCallback = function(displayName, note)
                        SetGuildBlacklistNote(self.guildId, data.index, note)
                    end
                }
...
function ZO_GuildRecruitment_Blacklist_Keyboard:Row_Remove_OnClicked(control)
    local data = ZO_ScrollList_GetData(control)
    if data then
        RemoveFromGuildBlacklist(data.guildId, data.index)
    end
end
...
function ZO_GuildRecruitment_AddToBlacklistDialog_Keyboard_OnInitialized(self)
    local function UpdateAddRestrictions(nameEditControl)
        ...
        if not GUILD_RECRUITMENT_MANAGER:IsBlacklistResultSuccessful(result) then
        ...
        buttons =
        {
            -- Yes Button
            {
                ...
                callback = function(dialog)
                    ...
                    local blacklisted = AddToGuildBlacklist(dialog.data.guildId, name, note)
                    if not GUILD_RECRUITMENT_MANAGER:IsBlacklistResultSuccessful(blacklisted) then
                        ZO_Dialogs_ShowPlatformDialog("GUILD_FINDER_BLACKLIST_FAILED", nil, { mainTextParams = { blacklisted } })
                    end
                end,
            },
            -- No Button
