        ...
        parametricList =
        {
            ...
            -- Add to Blacklist
            {
                ...
                templateData = {
                    ...
                    callback = function(dialog)
                        local blacklisted = AddToGuildBlacklist(dialog.data.guildId, dialog.data.name, dialog.data.note)
        ...
        {
            {
                ...
                enabled = function(dialog)
                    ...
                    elseif targetData.confirmEntry then
                        ...
                        if GUILD_RECRUITMENT_MANAGER:IsBlacklistResultSuccessful(result) then
                            return true
                        else
        ...
        parametricList =
        {
            -- Remove from Blacklist
            {
                ...
                templateData = {
                    ...
                    setup = ZO_SharedGamepadEntry_OnSetup,
                    callback = function(dialog)
                        RemoveFromGuildBlacklist(dialog.data.guildId, dialog.data.index)
                        ReleaseDialog()
                    end,
                },
            },
                ...
                templateData = {
                    ...
                    callback = function(dialog)
                        local function NoteChangedCallback(displayName, note)
                            SetGuildBlacklistNote(self.guildId, dialog.data.index, note)
                        end
