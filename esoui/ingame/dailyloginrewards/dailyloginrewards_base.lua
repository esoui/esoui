...
function ZO_DailyLoginRewards_Base:Initialize(control)
    ...
    self.lastCalcualtedTimeUntilNextMonthS = 0
    ...
    control:RegisterForEvent(EVENT_NEW_DAILY_LOGIN_REWARD_AVAILABLE, function(...) self:OnNewDailyLoginReward(...) end)
...
function ZO_DailyLoginRewards_Base:UpdateTimeToNextMonth()
    ...
    if timeUntilNextDailyLoginMonthS ~= self.lastCalcualtedTimeUntilNextMonthS then
        local formattedTime = ZO_FormatTimeLargestTwo(timeUntilNextDailyLoginMonthS, TIME_FORMAT_STYLE_DESCRIPTIVE_MINIMAL)
        self.lastCalcualtedTimeUntilNextMonthS = timeUntilNextDailyLoginMonthS
...
function ZO_DailyLoginRewards_Base:OnNewDailyLoginReward()
    self:MarkDirty()
end
