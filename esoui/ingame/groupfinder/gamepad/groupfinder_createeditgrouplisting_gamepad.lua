        ...
        templateData =
        {
            focusLostCallback = function(control)
                ...
                if championPoints and championPoints ~= 0 then
                    self.userTypeData:SetChampionPoints(tonumber(championPoints))
                end
                local dialog = ZO_GenericGamepadDialog_GetControl(GAMEPAD_DIALOGS.PARAMETRIC)
                ZO_GenericGamepadDialog_RefreshKeybinds(dialog)
            end,
                ...
                setup = function(control, data, selected, reselectingDuringRebuild, enabled, active)
                    local function GetRoleMin(spinner)
                        if spinner.roleType == LFG_ROLE_INVALID then
                            return self.userTypeData:GetDesiredRoleCount(LFG_ROLE_INVALID)
                        else
                    ...
                    control.roleSpinner:SetValue(self.userTypeData:GetDesiredRoleCount(roleType))
...
function ZO_GroupFinder_CreateEditGroupListing_Gamepad:UpdateRoles()
    for roleType, control in pairs(self.roleSpinnerTable) do
        if control and control.roleSpinner and not control.roleSpinner:SetValue(self.userTypeData:GetDesiredRoleCount(roleType)) then
